...
describe("tools", () => {
  ...
  describe("listFile", () => {
    test("ディレクトリの内容を取得できること", async () => {
      ...
      const result = await listFile({ path: ".", recursive: true });
    ...
    test("存在しないディレクトリを指定するとエラーを返すこと", async () => {
      const result = await listFile({ path: "not_exists", recursive: true });
  ...
  describe("readFile", () => {
    ...
    test("存在しないファイルを指定するとエラーを返すこと", async () => {
      const result = await readFile({ path: "not_exists.txt" });
      expect(result.ok).toBe(false);
      if (!result.ok) {
